...
Requirements:
    ...
    pip install scipy
文章连接： https://mp.weixin.qq.com/s?__biz=MzAxMjUyNDQ5OA==&mid=2653560898&idx=1&sn=99cf0272758ca4cd7e14a9a1961ff626&chksm=806e0effb71987e93fbfdce205bb3da5b6b915826b27bc64d264106c5c181e2baaf889e23add&mpshare=1&scene=24&srcid=&sharer_sharetime=1570495348437&sharer_shareid=4e9f7dd5a6cc79f5f4d1cb18e52a5887#rd
"""
...
class BicycleTrack:
    ...
    def solve(self, span, P0, num=100):
        """
        solve back wheel track according to front wheel track and initial
        ...
        # front wheel track
        self.FX, self.FY = \
            self.front_track_x(self.t), self.front_track_y(self.t)

    def plot(self, plt, front_style='-', front_color='deepskyblue',
             back_style='-', back_color='tomato'):
    ...
    def animate(self, plt, animation, front_color='deepskyblue',
                back_color='tomato'):
        """
        ...
        plt.legend((front_track, back_track),
                   ('$Front Wheel Track$', '$Back Wheel Track$'))
    ...
    def __pos(self, P, Q, tyre_ratio=0.4, handle_ratio=0.5):
        ...
        # front_wheel:
        front_pos = ([x + r_tyre * vx, x - r_tyre * vx],
                     [y + r_tyre * vy, y - r_tyre * vy])

        # back_wheel:
        back_pos = ([x1 + r_tyre * vx, x1 - r_tyre * vx],
                    [y1 + r_tyre * vy, y1 - r_tyre * vy])

        # handlebar:
        handle_pos = ([x + r_handle * fx, x - r_handle * fx],
                      [y + r_handle * fy, y - r_handle * fy])
    ...
    def __update_pos(self, PQ, lines):
        ...
        Argument:
            ...
            lines: line objects representing bicycle components, e.g. frame,
            wheel, handlebar
        Return:
        ...
        # get new positions
        frame_pos, front_pos, back_pos, handle_pos = self.__pos((x1, y1),
                                                                (x, y))

        # update bicycle positions
        if frame:
            frame.set_data(*frame_pos)
        if front:
            front.set_data(*front_pos)
        if back:
            back.set_data(*back_pos)
        if handle:
            handle.set_data(*handle_pos)

        # update tracks
        # the animation is repeating, so needn't to update the track once
        # finished in a loop
        if front_track:
            fx, fy = front_track.get_data()
            # add new track point in the first loop
            fx = np.append(fx, x) if fx.shape[0] < self.FX.shape[0] else \
                np.array([])
            fy = np.append(fy, y) if fy.shape[0] < self.FY.shape[0] else \
                np.array([])
            front_track.set_data(fx, fy)

        if back_track:
            bx, by = back_track.get_data()
            bx = np.append(bx, x1) if bx.shape[0] < self.X.shape[0] else \
                np.array([])
            by = np.append(by, y1) if by.shape[0] < self.Y.shape[0] else \
                np.array([])
...
if __name__ == '__main__':
    ...
    def fx(t):
        return (t <= 0) * f_half_circle(t)[0] + (t > 0) * (t <= 2 * np.pi) * \
            f_whole_circle(t)[0] + (t > 2 * np.pi) * f_line(t)[0]

    def fy(t):
        return (t <= 0) * f_half_circle(t)[1] + (t > 0) * (t <= 2 * np.pi) * \
            f_whole_circle(t)[1] + (t > 2 * np.pi) * f_line(t)[1]
